...
pub fn duma_protocol_version() -> String {
...
impl ParsedVersion {
    pub fn new(version: &str) -> Result<Self, duma::Error> {
        let parts: Vec<&str> = version.split('.').collect();
        if parts.len() != 2 {
            Err(duma::Error::InvalidVersion)
        } else {
            let major = parts[0]
                .parse::<i32>()
                .map_err(|_| duma::Error::InvalidVersion)?;
            let minor = parts[1]
                .parse::<i32>()
                .map_err(|_| duma::Error::InvalidVersion)?;
            Ok(Self { major, minor })
...
pub fn get_highest_supported_version_for_major_version(
    ...
    if *major_version != MAJOR_VERSION {
        ...
        ParsedVersion::new(&duma_protocol_version()).ok()
    }
}
...
pub fn select_lower_version(version1: &str, version2: &str) -> Result<String, duma::Error> {
