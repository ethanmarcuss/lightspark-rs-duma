...
mod tests {
    ...
    use crate::duma::{
        ...
        is_duma_lnurl_query, parse_lnurlp_request, parse_lnurlp_response, parse_pay_req_response,
        parse_pay_request, verify_pay_req_signature, verify_duma_lnurl_query_signature,
        verify_duma_lnurlp_response_signature, UmaInvoiceCreator,
    };
    ...
    fn test_parse() {
        ...
        let expected_query = LnurlpRequest {
            ...
            timestamp,
            duma_version: "0.1".to_string(),
        };

        let url_string = format!("https://vasp2.com/.well-known/lnurlp/bob?signature=signature&nonce=12345&vaspDomain=vasp1.com&dumaVersion=0.1&isSubjectToTravelRule=true&timestamp={}",&timestamp);
    ...
    #[test]
    fn test_is_duma_query_valid() {
        let url_string = "https://vasp2.com/.well-known/lnurlp/bob?signature=signature&nonce=12345&vaspDomain=vasp1.com&dumaVersion=0.1&isSubjectToTravelRule=true&timestamp=12345678";
        let url = url::Url::parse(url_string).unwrap();
        assert!(is_duma_lnurl_query(&url));
    }

    #[test]
    fn test_is_duma_query_missing_params() {
        let url_string = "https://vasp2.com/.well-known/lnurlp/bob?nonce=12345&vaspDomain=vasp1.com&dumaVersion=0.1&isSubjectToTravelRule=true&timestamp=12345678";
        let url = url::Url::parse(url_string).unwrap();
        assert!(!is_duma_lnurl_query(&url));

        let url_string = "https://vasp2.com/.well-known/lnurlp/bob?signature=signature&vaspDomain=vasp1.com&dumaVersion=0.1&isSubjectToTravelRule=true&timestamp=12345678";
        let url = url::Url::parse(url_string).unwrap();
        assert!(!is_duma_lnurl_query(&url));

        let url_string = "https://vasp2.com/.well-known/lnurlp/bob?signature=signature&nonce=12345&dumaVersion=0.1&isSubjectToTravelRule=true&timestamp=12345678";
        let url = url::Url::parse(url_string).unwrap();
        assert!(!is_duma_lnurl_query(&url));

        let url_string = "https://vasp2.com/.well-known/lnurlp/bob?signature=signature&nonce=12345&dumaVersion=0.1&vaspDomain=vasp1.com&timestamp=12345678";
        let url = url::Url::parse(url_string).unwrap();
        // isSubjectToTravelRule is optional
        assert!(is_duma_lnurl_query(&url));

        let url_string = "https://vasp2.com/.well-known/lnurlp/bob?signature=signature&nonce=12345&dumaVersion=0.1&vaspDomain=vasp1.com&isSubjectToTravelRule=true";
        let url = url::Url::parse(url_string).unwrap();
        assert!(!is_duma_lnurl_query(&url));
    ...
    #[test]
    fn test_is_duma_query_invalid_path() {
        let url_string = "https://vasp2.com/.well-known/lnurla/bob?signature=signature&nonce=12345&vaspDomain=vasp1.com&dumaVersion=0.1&isSubjectToTravelRule=true&timestamp=12345678";
        let url = url::Url::parse(url_string).unwrap();
        assert!(!is_duma_lnurl_query(&url));

        let url_string = "https://vasp2.com/bob?signature=signature&nonce=12345&vaspDomain=vasp1.com&dumaVersion=0.1&isSubjectToTravelRule=true&timestamp=12345678";
        let url = url::Url::parse(url_string).unwrap();
        assert!(!is_duma_lnurl_query(&url));

        let url_string = "https://vasp2.com/?signature=signature&nonce=12345&vaspDomain=vasp1.com&dumaVersion=0.1&isSubjectToTravelRule=true&timestamp=12345678";
        let url = url::Url::parse(url_string).unwrap();
        assert!(!is_duma_lnurl_query(&url));
    }
    ...
    fn test_sign_and_verify_lnurlp_request() {
        ...
        let result = verify_duma_lnurl_query_signature(query, &pk.serialize());
        assert!(result.is_ok());
    }
    ...
    fn test_sign_and_verify_lnurlp_response() {
        ...
        let result = verify_duma_lnurlp_response_signature(response, &pk2.serialize());
        assert!(result.is_ok());
    }
